...
export interface DataMapping {
  ...
  source?: (properties: Record<string, unknown>) => unknown;
...
export const featureConfig: Record<string, FeatureTypeConfig> = {
  state: {
    ...
    dataMapping: {
      '[state-list-item="image"]': {
        type: "image",
        source: (p: Record<string, unknown>) =>
          p["IMAGE"] ||
      ...
      '[state-list-item="title"]': {
        type: "text",
        source: (p: Record<string, unknown>) => p.NAME || "Unnamed State",
      },
    },
  },
  region: {
    ...
    dataMapping: {
      ...
      '[city-list-item="title"]': {
        type: "text",
        source: (p: Record<string, unknown>) => p["name"] || "Unnamed Region",
      },
    },
  },
  beach: {
    ...
    dataMapping: {
      ...
      '[beach-list-item="title"]': {
        type: "text",
        source: (p: Record<string, unknown>) => p.Name || "Beach Title",
      },
      '[beach-list-item="location-cluster"]': {
        type: "text",
        source: (p: Record<string, unknown>) => p["Location Cluster"],
      },
      '[beach-list-item="state"]': { type: "text", source: (p: Record<string, unknown>) => p.State },
  ...
  poi: {
    ...
    dataMapping: {
      ...
      '[beach-list-item="title"]': {
        type: "text",
        source: (p: Record<string, unknown>) => p.name || p.Name || "Point of Interest",
      },
      '[beach-list-item="location-cluster"]': {
        type: "text",
        source: (p: Record<string, unknown>) => p.categoryName || p.category || p.type || "POI",
      },
      '[beach-list-item="state"]': { 
        type: "text", 
        source: (p: Record<string, unknown>) => p.customIconName || p["Custom Icon"] || p.State || p.state || "" 
      },
      '[beach-list-item="delimiter"]': {
        type: "style",
        style: { display: "inline" },
