...
function transformPOIs(pois, maps) {
    ...
    return pois.map(item => {
        ...
        transformedPOI.name = transformedPOI.name || transformedPOI.Name || 'Unnamed POI';
        transformedPOI.slug = transformedPOI.slug || transformedPOI.Slug || '';
        
        // Parse coordinates
        transformedPOI.longitude = parseCoordinate(transformedPOI.longitude || transformedPOI.Longitude);
        transformedPOI.latitude = parseCoordinate(transformedPOI.latitude || transformedPOI.Latitude);
        ...
        if (transformedPOI['custom-icon'] && iconMap.has(transformedPOI['custom-icon'])) {
            transformedPOI.customIconName = iconMap.get(transformedPOI['custom-icon']);
        } else {
            transformedPOI.customIconName = transformedPOI['custom-icon'] || 'default';
        }
        
        // Handle image fields
        transformedPOI.mainImageUrl = transformedPOI['main-image']?.url || null;
        transformedPOI.mainImageAlt = transformedPOI['main-image']?.alt || transformedPOI.name;
        
        // Handle rich text content
        transformedPOI.richTextContent = transformedPOI['rich-text'] || transformedPOI.description || '';
...
async function fetchAllPOIs() {
    ...
    do {
        ...
        if (!response.ok) {
            ...
            console.error(`[api/pois] Webflow API Error: ${response.status}`, errorData);
...
export default async function handler(req, res) {
    try {
        ...
        console.log(`[api/pois] Successfully fetched and transformed ${formattedPOIs.length} POI items.`);
        ...
        const mockPOIData = [
            {
                ...
                richTextContent: "<p>Professional lifeguard station providing safety services and emergency response at Huntington City Beach. Staffed during peak hours with certified lifeguards.</p>",
                geometry: {
                    type: 'Point',
            ...
            },
            {
                ...
                mainImageUrl: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400",
                mainImageAlt: "Santa Monica Pier Information Center",
                richTextContent: "<p>Visitor information center providing maps, event schedules, and assistance for Santa Monica Pier attractions and activities.</p>",
                geometry: {
                    type: 'Point',
            ...
            },
            {
                ...
                mainImageUrl: "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400",
                mainImageAlt: "Venice Beach Skate Park",
                richTextContent: "<p>Famous concrete skate park featuring bowls, ramps, and street course elements. Open to skateboarders, BMX riders, and roller skaters.</p>",
                geometry: {
                    type: 'Point',
                    coordinates: [-118.4681, 33.9850]
                }
            }
        ];
